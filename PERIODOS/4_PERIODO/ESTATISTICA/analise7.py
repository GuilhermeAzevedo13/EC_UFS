...
print(n_array_reshape.shape)

print(n_array_reshape)

print("\n")

print("Array N em Rol\n")
n_array_reshape = np.sort(n_array_reshape, axis=0)
print(n_array_reshape)
print("\n")
print(n_array_reshape.shape)
print("\n")
...
soma_valores_n = np.sum(n_array_reshape)
print("Soma dos valores de N:", soma_valores_n)
print("\n")
media = soma_valores_n/362
print("Média dos Valores N: {:.2f}".format(media))

# Calcula a mediana dos valores na matriz akp_array_reshape
mediana = np.median(n_array_reshape)

# Exibe a mediana com duas casas decimais
print(f"Mediana dos valores de N: {round(mediana, 2)}")
...
# Exemplo de uso com n_array_reshape
# flattten() transforma o shape de bidimensional (362,1) para unidimensional com 362 valores
valores_repetidos = valores_mais_repetidos(n_array_reshape.flatten())
print(f"Valores mais repetidos: {valores_repetidos}")

print("\n")
variancia = np.var(n_array_reshape)
desvio_padrao = np.std(n_array_reshape)
coeficiente_variacao = np.std(
    n_array_reshape) / np.mean(n_array_reshape) * 100
...
valor_maximo = np.max(n_array_reshape)
valor_minimo = np.min(n_array_reshape)
