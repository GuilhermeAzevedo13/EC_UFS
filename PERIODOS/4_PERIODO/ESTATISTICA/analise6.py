...
alb_array = base['ALB'].values

# Printa o shape da coluna Grupo: (362,)
print(alb_array.shape)

# Faz um reshape para reconhecer ela como apenas uma coluna
alb_array_reshape = alb_array.reshape(-1, 1)
# Printa o shape corrigido: (362,1)
print(alb_array_reshape.shape)

print(alb_array_reshape)

print("\n")

print("Array ALB em Rol\n")
alb_array_reshape = np.sort(alb_array_reshape, axis=0)
print(alb_array_reshape)
print("\n")
print(alb_array_reshape.shape)
print("\n")
...
soma_valores_alb = np.sum(alb_array_reshape)
print("Soma dos valores de ALB:", soma_valores_alb)
print("\n")
media = soma_valores_alb/362
print("Média dos Valores ALB: {:.2f}".format(media))

# Calcula a mediana dos valores na matriz akp_array_reshape
mediana = np.median(alb_array_reshape)

# Exibe a mediana com duas casas decimais
print(f"Mediana dos valores de ALB: {round(mediana, 2)}")
...
# Exemplo de uso com alb_array_reshape
# flattten() transforma o shape de bidimensional (362,1) para unidimensional com 362 valores
valores_repetidos = valores_mais_repetidos(alb_array_reshape.flatten())
print(f"Valores mais repetidos: {valores_repetidos}")

print("\n")
variancia = np.var(alb_array_reshape)
desvio_padrao = np.std(alb_array_reshape)
coeficiente_variacao = np.std(
    alb_array_reshape) / np.mean(alb_array_reshape) * 100
...
valor_maximo = np.max(alb_array_reshape)
valor_minimo = np.min(alb_array_reshape)
